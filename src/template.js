/**
 * template.js — a deliberately tiny mustache-shaped renderer.
 *
 * Enough for the runbook and dashboard templates, small enough to read in one
 * sitting. No dependencies, no code execution, no partials.
 *
...
const trimStandaloneTags = (tpl) => tpl.replace(/^[ \t]*(\{\{[#^/][^}]+\}\})[ \t]*\r?\n/gm, '$1');

export function render(template, data, { escape = (s) => s } = {}) {
  const run = (tpl, scopes) => {
    let out = tpl;
    ...
    for (let m = SECTION.exec(out); m; m = SECTION.exec(out)) {
      ...
      if (kind === '#') {
        if (Array.isArray(value)) {
          ...
          replacement = run(body, [...scopes, typeof value === 'object' ? value : {}]);
        }
      } else if (isEmpty(value)) {
