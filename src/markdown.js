/**
 * markdown.js — a documented subset of Markdown → HTML.
 *
 * Only what the runbooks actually use, so the dashboard can show a runbook
 * inline on a phone instead of sending the owner to GitHub mid-session:
 * ATX headings, fenced code, unordered and ordered lists, blockquotes,
 * pipe tables, paragraphs, and inline `code`, **bold**, *italic*, links.
 *
...
export function markdownToHtml(md, { headingOffset = 0 } = {}) {
  ...
  while (i < lines.length) {
    ...
    if (/^\s*[-*]\s+/.test(line) || /^\s*\d+\.\s+/.test(line)) {
      ...
      while (i < lines.length && (ordered ? /^\s*\d+\.\s+/ : /^\s*[-*]\s+/).test(lines[i])) {
        ...
        const startsItem = (line) => /^([-*]\s+|\d+\.\s+)/.test(line.trim());
